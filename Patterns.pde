...
class Tribal extends LXPattern {
  ...
  boolean isInRing(LXPoint p, int idx) {
    ...
    return distance >= (idx + 1) * 25 * zoom * fxscale && distance <= (((idx + 1) * 25) + 14) * zoom * fxscale; 
  }
  ...
  float glowAt(float angle, float distance) {
    ...
    float glow = constrain(sin(glowAngle*3), 0, 1) * mask * constrain(1. - distance / (200. * fxscale), 0, 1); 
...
public class TextScroller extends ImageMappedPattern {
  ...
  BasicParameter speed = new BasicParameter("SPEED", 1, 0.1, 3);
...
public class Grow extends ImageMappedPattern {
  ...
  void drawImage() {
    ...
    g.strokeWeight(15 * 2);
    g.noFill();
    g.line(-model.xRange * 2, 0, model.xRange * 2, 0);
...
class BlockBase extends LXPattern {
  final BasicParameter speed = new BasicParameter("SPEED", 0.25*SECONDS, 0.01*SECONDS, 0.5*SECONDS);
  final BasicParameter blockWidth = new BasicParameter("BLOCKWIDTH", 10, 5, model.xRange);
  final BasicParameter blockHeight = new BasicParameter("BLOCKHEIGHT", 10, 5, model.yRange);
  ...
  BlockBase(LX lx) {
    ...
    println(speed.getValuef(), speed.range.v0, speed.range.min, speed.range.v1, speed.range.max);
...
class BlockRandom extends BlockBase {
  final BasicParameter decay = new BasicParameter("DECAY", 0.025, 0.005, 0.05);
...
class BlockSpiral extends BlockBase {
  ...
  BlockSpiral(LX lx) {
    ...
    blockWidth.setValue(5.0f);
    blockHeight.setValue(5.0f);
  }
